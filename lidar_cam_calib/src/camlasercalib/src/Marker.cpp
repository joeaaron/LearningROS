...
const int Marker::m_idVerify[4][3] = {{0,1,0},
									  {0,1,1},
									  {1,0,0},
									  {1,1,1}};	// The 4 word id:00 01 10 11
...
int Marker::hammDistMarker(const Mat& _code)
{
	...
	for (int i=0; i<3; i++)
	{
		...
		for (int x=0; x<4; x++)	 		//match each word to find the nearest word
		{
			int sumTemp = 0;
			for (int j=0; j<3; j++)
			{
...
int Marker::decode(Mat& _input, int& _numRotation)
{
	...
	// judge if the surroundding is all black
	for (int i=0; i<5; i++)
	{
		int step = 4;
		if (i==0 || i==4)
